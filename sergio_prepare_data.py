...
- SERGIO's "bins" (originally meant as distinct cell TYPES) are relabeled
  ...
  GRN. All cells get a single shared cell_type label ("sim"), which lets
  Marlene's MAML loop run (it needs >=1 class/task) without asserting a
  cell-type distinction that isn't scientifically present in this data.
...
def load_sergio_dataset(
    ...
    return X, timepoint, gene_ids, gt_edges
...
def main():
    ...
    ap.add_argument(
        "--n_replicates", type=int, default=1,
        help="How many of the 15 pre-simulated replicate runs to use "
             "(more replicates = more cells, slower). Start with 1.",
    )
    ap.add_argument(
    ...
    X, timepoint, gene_ids, gt_edges = load_sergio_dataset(
    ...
    if args.n_timepoints_keep is not None:
        ...
        X, timepoint = X[mask], timepoint[mask]
    ...
    adata = anndata.AnnData(
        ...
        obs=pd.DataFrame({
            ...
            "cell_type": "sim",  # single synthetic cell type, see module docstring
        }),
